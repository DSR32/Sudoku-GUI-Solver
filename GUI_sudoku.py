...
from Sudoku_generator import valid, solve, find_empty
...
def generate(difficulty):
    '''Randomly generates a Sudoku grid/board based on the difficulty level'''
    while True:  # return will interrupt the loop
        ...
        if solve(board):
            if difficulty == 'easy':
                ...
                # Remove 60 cells for hard difficulty
                remove_count = 60
            else:
            ...
            indices = random.sample(range(81), remove_count)
...
def main():
    ...
    wrong = 0
    difficulty = None  # Initialize difficulty as None

    while difficulty is None:  # Loop until difficulty is selected
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                return
                ...
                if 180 <= mouse_pos[0] <= 360 and 240 <= mouse_pos[1] <= 300:
                    difficulty = DIFFICULTY_EASY
                elif 180 <= mouse_pos[0] <= 360 and 320 <= mouse_pos[1] <= 380:
                    difficulty = DIFFICULTY_MEDIUM
                elif 180 <= mouse_pos[0] <= 360 and 400 <= mouse_pos[1] <= 460:
                    difficulty = DIFFICULTY_HARD

        # Draw buttons for difficulty selection
        pygame.draw.rect(screen, (1, 1, 1), pygame.Rect(180, 240, 180, 60))  # Easy button
        pygame.draw.rect(screen, (1, 1, 1), pygame.Rect(180, 320, 180, 60))  # Medium button
        pygame.draw.rect(screen, (1, 1, 1), pygame.Rect(180, 400, 180, 60))  # Hard button

        font = pygame.font.SysFont('Brush Script', 40)
        text_easy = font.render('Easy', True, (240, 255, 255))
        text_medium = font.render('Medium', True, (240, 250, 255))
        text_hard = font.render('Hard', True, (240, 250, 255))

        screen.blit(text_easy, (235, 248))
        screen.blit(text_medium, (215, 328))
        screen.blit(text_hard, (232, 408))

        pygame.display.flip()

    # Create the Sudoku board based on the selected difficulty
